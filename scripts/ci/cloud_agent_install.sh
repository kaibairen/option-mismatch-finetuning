...
if command -v nvidia-smi >/dev/null 2>&1 && nvidia-smi >/dev/null 2>&1; then
  ...
  python3 -m pip install -r "$ROOT/requirements/gpu.txt"
else
...
python3 "$ROOT/scripts/data/prepare_data.py" --split test --limit 50
python3 "$ROOT/scripts/data/prepare_data.py" --split train --limit 256
